...
def pressure_ready(turn):
    base=turn.station()
    towers=turn.weapons()
    roles=turn.controllable()
    return bool(turn.is_day and base and base.level>=2
        and base.health>=.85*1500*base.level and len(towers)==3
        and all(t.level>=2 and t.health>=.75*(500+500*t.level) for t in towers)
        and len(roles)==3 and all(u.health>=.75*(200 if u.kind=='pioneer' else 220) for u in roles)
        and not turn.incoming_robots()
        and any(u.kind=='station' and u.level>=3 and 0<u.health<=300 for u in turn.enemies))
...
def use_carried(turn, role, claimed, commands, night=False):
    ...
    if medicine and role.health < max_hp*.65:
        ...
        return True
    if (not night and role.kind=='pioneer' and (turn.round_no-1)%130==60
            and 'MiddleRobotSummonOrder' in role.backpack and pressure_ready(turn)):
        commands[role.unit_id]={'action':'use','name':'MiddleRobotSummonOrder'}
        return True
...
def develop(turn, role, claimed, commands):
    ...
    if (role.kind=='pioneer' and can_shop and not role.backpack_full
            and 48 <= (turn.round_no-1)%130 <= 50 and pressure_ready(turn)
            and not any(n.endswith('RobotSummonOrder') for n in stock)
            and price>0 and budget>=reserve+150+price
            and any(distance(role.pos,p)<=1 for p in shops)):
