...
from .guard import safer_step, exposure, station_side_rank, joint_positions, retreat_step, shot_relieves_threat
...
def _night(turn: Turn, commands: dict[int, dict[str, Any]], policy: dict | None = None,
    ...
    for role, tower in sorted(pairs, key=lambda pair: pair[1].range_of_attack()):
        ...
        if distance(role.pos, tower.pos) <= 1:
            ...
            if step is not None and smart and tower.cooldown==0 and tower.kind=="railgun":
                # Preview is isolated: moving must not reserve imaginary damage.
                preview=dict(reserved)
                planned=defense_targets(turn,tower,preview)
                if planned and shot_relieves_threat(turn,role,reserved,preview):
                    command=attack_command(role.unit_id,planned[0])
                    command["targetPos"]=[pos.dump() for pos in planned]
            ...
            if targets:
                ...
                commands[tower.unit_id] = command
            continue
        step=retreat_step(turn,role,claimed)
        if step is not None:
            commands[role.unit_id]=move_command(step)
            claimed.add(step)
            continue
