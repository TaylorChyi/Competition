...
def develop(turn, role, claimed, commands):
    ...
    # A wounded operator already at the shop can buy without a detour.
    # Preserve the entire next station voucher and earlier shared-wallet orders.
    max_hp = 200 if role.kind=='pioneer' else 220
    if (turn.is_day and can_shop and 0<role.health<.65*max_hp
            and medicine_price>0 and not role.backpack_full
            and 'Medicine' not in role.backpack
            and any(distance(role.pos,p)<=1 for p in shops)
            and budget>=medicine_price+reserve):
