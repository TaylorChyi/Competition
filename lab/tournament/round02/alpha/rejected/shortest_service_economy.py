...
def route(turn, role, locations, claimed):
    ...
    def shortest_service_path(avoid):
        best=None
        for goal in goals:
            # Sorted geometric distance is a lower bound: later goals cannot
            # improve a route already this short. Preserve stable tie choices.
            if best is not None and distance(role.pos,goal)>=len(best):
                break
            if goal in avoid:
                continue
            candidate=shortest_path(turn,role,goal,avoid,reroute=False)
            if candidate is not None and (best is None or len(candidate)<len(best)):
                best=candidate
        return best

    path=shortest_service_path(claimed)
    if path and role.unit_id in turn.failed_actions:
        alternative=shortest_service_path(claimed | {path[0]})
