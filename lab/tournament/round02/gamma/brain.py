...
def _worker_day(
    ...
    if economy and turn.shop_prices:
        ...
        if (fortify and base and not base_under_fire and not distant_opening_stone
            ...
            # Preserve the opening screen; after the base upgrade, use scarce
            # worker turns on gun upgrades instead of repairing every flank.
            screen = min(fortify,2) if base.level>=2 else fortify
            front=sorted(_wall_order(turn),key=lambda p:((p.x-cx)**2+(p.y-cy)**2,p.x,p.y))[:screen]
