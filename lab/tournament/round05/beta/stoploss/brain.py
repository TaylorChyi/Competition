...
from .guard import safer_step, exposure, station_side_rank, joint_positions, retreat_step
...
def _night(turn: Turn, commands: dict[int, dict[str, Any]], policy: dict | None = None,
    ...
    base=turn.station()
    # Use the existing economy emergency boundary: preserve established gunfire
    # when the station itself urgently needs protection.
    base_emergency=base is not None and base.health<.75*1500*base.level
    coordinated=None if base_emergency else joint_positions(turn,pairs,claimed)
    ...
    for role, tower in sorted(pairs, key=lambda pair: pair[1].range_of_attack()):
        ...
        if distance(role.pos, tower.pos) <= 1:
            step = None if coordinated is not None else safer_step(turn,role,tower,claimed,anticipate=not base_emergency)
            if step is not None:
                commands[role.unit_id] = move_command(step)
                claimed.add(step)
                continue
