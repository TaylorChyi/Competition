...
def joint_positions(turn, pairs, claimed):
    ...
    if not any(exposure(turn,role.pos,approaching=False)>=role.health for role in roles):
        return None
    ...
    for positions in product(*options):
        ...
        if any(positions[i]==roles[j].pos and positions[j]==roles[i].pos
            ...
            continue
        fatal=sum(risk[p][0]>=r.health for r,p in zip(roles,positions))
    ...
    if best is None:
        return None
    current_fatal=sum(exposure(turn,r.pos,approaching=False)>=r.health for r in roles)
