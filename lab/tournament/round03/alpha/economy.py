...
def base_refit_needed(turn):
    base=turn.station()
    return bool(base and base.health>0 and base.level<3 and (
        base.health<.75*1500*base.level or
        (base.level==1 and turn.round_no>130 and base.health<1500)))


def repair_upgrade(turn, commands):
    """An affordable, deliverable level-two repair; do not displace base rescue."""
    if not turn.is_day or base_refit_needed(turn):
        return None
    name='WeaponUpgradeVoucher2'
    price=turn.shop_prices.get(name,0)
    if price<=0 or cash_available(turn,commands)<price:
        return None
    if not any(t.level==2 and t.health<250 for t in turn.weapons()):
        return None
    roles=turn.controllable()
    couriers=[r for r in roles if r.kind=='pioneer'] or list(turn.workers()[-1:])
    if not couriers or couriers[0].backpack_full or couriers[0].health<50:
        return None
    shops=[p for p,k in turn.zones.items() if k=='weaponShop']
    shopping_path=purchase_route(turn,couriers[0],shops,set())
    if shopping_path is None:
        return None
    buyer=replace(couriers[0],pos=shopping_path[-1] if shopping_path else couriers[0].pos)
    target=min(upgrade_targets(turn,name),key=lambda t:(t.kind!='rocket',
               t.health/(500+500*t.level),distance(buyer.pos,t.pos),t.unit_id))
    if target.health>=250:
        return None
    delivery=route(turn,buyer,[target.pos],set())
    if delivery is None or len(shopping_path)+len(delivery)+3>70-(turn.round_no-1)%130:
        return None
    return name


def sell_at_vendor_for_upgrade(turn,role,commands):
    """Bank immediately spendable ore before optional maintenance, no detour."""
    if not any(k=='vendor' and distance(role.pos,p)<=1 for p,k in turn.zones.items()):
        return False
    stock=Counter(n for r in turn.controllable() for n in r.backpack)
    names=[]
    if base_refit_needed(turn):
        names.append('StationUpgradeVoucher'+str(turn.station().level))
    if any(t.level==2 and t.health<250 for t in turn.weapons()):
        names.append('WeaponUpgradeVoucher2')
    names.extend('WeaponUpgradeVoucher'+str(level) for level in (1,2)
                 if any(t.level==level for t in turn.weapons()))
    name=next((n for n in names if len(upgrade_targets(turn,n))>stock[n]
               and turn.shop_prices.get(n,0)>0),None)
    if name is None:
        return False
    gap=turn.shop_prices[name]-cash_available(turn,commands)
    inventory=Counter(n for n in role.backpack if n in ORE)
    # One sale must suffice; mixed ore that needs several actions keeps the
    # regular economic planner's behavior rather than promising instant funds.
    choices=[n for n,count in inventory.items()
             if 0<gap<=count*turn.sale_prices.get(n,0)]
    if not choices:
        return False
    ore=max(choices,key=lambda n:(inventory[n]*turn.sale_prices[n],n))
    commands[role.unit_id]={'action':'sell','name':ore,'num':inventory[ore]}
    return True
...
def develop(turn, role, claimed, commands):
    ...
    repair_name=repair_upgrade(turn,commands)
    if repair_name:
        reserve=max(reserve,turn.shop_prices[repair_name])
