...
def targets(turn, tower, reserved):
    ...
    # When the station is near destruction, its immediate damage outweighs
    # hypothetical operator exposure; retain ordinary scoring while healthy.
    station_weight = 1 + min(4, max(0, 1500/max(1,base.health)-1)) if base else 1
    urgency = {r.robot_id: station_weight*POWER.get(r.kind, 5) /
