...
def targets(turn, tower, reserved):
    ...
    base = turn.station()
    # Survival precedes points in the official result; damaged stations
    # retain the original threat-only allocation until repaired.
    bounty_weight = KILL_REWARD if base is None or base.health>.75*1500*base.level else 0
            ...
            for r,hit in hits:
                ...
                if hp and r.robot_id in ids:
                    ...
                    if tower.kind=='rocket':
                        ...
                        value += (urgency[r.robot_id]+bounty_weight*POINTS.get(r.kind,1)) * (min(hit,hp)/max(10,hp) + (1 if hit>=hp else 0))
