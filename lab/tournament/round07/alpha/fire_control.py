...
def threat_weights(turn, fallback):
    ...
    structures=[u for u in turn.ours if u.health>0
                and u.kind in ('wall','station','railgun','gatling','rocket')]
    durable=[u for u in structures if u.health>sum(
        POWER.get(r.kind,5) for r in incoming
        if min(distance(r.pos,p) for p in turn.footprint(u))<=3)]
    for robot in incoming:
        # Keep operator urgency if nearer cover can break in this volley.
        # Confirmed nearest-unit attacks: a strictly closer living structure
        # absorbs this turn's hit. Keep every tied operator as a possibility.
        nearest=min((min(distance(robot.pos,p) for p in turn.footprint(unit))
                     for unit in durable),default=100)
        urgency[robot.robot_id] += sum(POWER.get(robot.kind,5)*min(3,100/max(1,role.health))
            for role in turn.controllable()
            if distance(robot.pos,role.pos)<=3 and distance(robot.pos,role.pos)<=nearest)
    return urgency
