    ...
    if economy and turn.shop_prices:
        ...
        if (fortify and base
            ...
            if needed:
                ...
                if mine is not None and role.backpack.count('stone')<min(10 if len(walls)>=4 else 4,len(needed)):
...
def _wall_order(turn: Turn) -> tuple[Pos, ...]:
    ...
    return tuple(pos for pos in order if pos not in entrances and turn.land(pos))
