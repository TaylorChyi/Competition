...
def urgent_refit(turn,role,claimed,commands):
    ...
    if turn.gold-spent<price:
        return False
    daylight=70-(turn.round_no-1)%130
    trip=min(max(0,distance(role.pos,p)-1)+max(0,distance(p,base.pos)-1)+2 for p in shops)
    if turn.is_day and daylight<trip and min(distance(role.pos,p) for p in shops)>1:
        return False
...
def develop(turn, role, claimed, commands):
    ...
    if (role.health<100 and not role.backpack_full and budget>=turn.shop_prices.get('Medicine',10)
            and 'Medicine' in turn.shop_prices):
        if any(distance(role.pos,p)<=1 for p in shops):
            commands[role.unit_id] = {'action':'buy','name':'Medicine','num':1}
            return True
        return walk(turn,role,shops,claimed,commands)
    reserve_base=turn.station()
    reserve_name=('StationUpgradeVoucher'+str(reserve_base.level)
                  if reserve_base and reserve_base.level<3 else None)
    medicine_reserve=turn.shop_prices.get(reserve_name,100)
    if ('Medicine' in turn.shop_prices and 'Medicine' not in role.backpack
            and not role.backpack_full and budget>=turn.shop_prices['Medicine']+medicine_reserve
    ...
    if base and base.level<3:
        name='StationUpgradeVoucher'+str(base.level)
        if base.health<1500*base.level:
            goals.insert(0,name)
        else:
            goals.append(name)
