...
def use_carried(turn, role, claimed, commands, night=False):
    ...
    for name in sorted(role.backpack, key=lambda n:(not n.startswith('Station'),n)):
        ...
        if unit.kind=='station':
            ...
            pressure = sum(power.get(r.kind,5) for r in turn.incoming_robots()
                           if min(distance(r.pos,p) for p in footprint)<=4)
        heal_at = max(200*unit.level, 3*pressure)
        if unit.kind=='station' and unit.health > heal_at:
            # Alpha's reserve-until-damaged idea, with a separate return rule:
            # carry the voucher home early instead of beginning a fatal late trip.
...
def urgent_refit(turn,role,claimed,commands):
    ...
    base=turn.station()
    if role.backpack_full or role.kind!='pioneer' or not base or base.level>=3 or not (
...
def develop(turn, role, claimed, commands):
    ...
    if not role.backpack_full and role.health<100 and budget>=turn.shop_prices.get('Medicine',10) and 'Medicine' in turn.shop_prices:
        if any(distance(role.pos,p)<=1 for p in shops):
            commands[role.unit_id] = {'action':'buy','name':'Medicine','num':1}
            return True
        return walk(turn,role,shops,claimed,commands)
    repair_base = turn.station()
    repair_reserve = turn.shop_prices.get('StationUpgradeVoucher'+str(repair_base.level),100) if repair_base and repair_base.level<3 and repair_base.health<.75*1500*repair_base.level else 100
    if ('Medicine' in turn.shop_prices and 'Medicine' not in role.backpack
            and budget >= turn.shop_prices['Medicine'] + repair_reserve
            and any(distance(role.pos,p)<=1 for p in shops) and not role.backpack_full):
        commands[role.unit_id] = {'action':'buy','name':'Medicine','num':1}
        return True
    base = turn.station()
    goals = []
    ...
    if role.unit_id==courier.unit_id:
        for name in goals:
            ...
            # Complete one rocket's unlimited-range upgrade before spreading
            # the same wallet over all three level-one emplacements.
            rocket_focus = name.startswith('Weapon') and any(t.kind=='rocket' for t in targets)
            needed = (1 if rocket_focus else len(targets))-stock[name]
            price = turn.shop_prices.get(name,0)
