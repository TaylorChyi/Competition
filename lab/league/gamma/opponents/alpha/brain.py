...
def _day(turn: Turn, commands: dict[int, dict[str, Any]], loadout: tuple[str, ...], cover: bool,
    ...
    for role, tower in _tower_pairs(turn):
        ...
        if daylight_left <= len(route) + RETURN_BUFFER or (role.kind == PIONEER and
...
def _night(turn: Turn, commands: dict[int, dict[str, Any]], policy: dict | None = None,
    ...
    if not turn.incoming_robots():
        # The documented night phase permits mining and trade. Only leave
        # defenses once the entire observed incoming wave is cleared.
        for role in turn.controllable():
            develop(turn,role,claimed,commands)
        return
    for role in turn.controllable():
    ...
    for role, tower in sorted(_tower_pairs(turn), key=lambda pair: pair[1].range_of_attack()):
        if role.unit_id in commands:
            continue
