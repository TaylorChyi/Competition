...
def use_carried(turn, role, claimed, commands, night=False):
    ...
    if medicine and role.health < max_hp*.65:
        ...
        return True
    # A single courier and ten fixed daytime slots guarantee the team daily cap
    # without persistent process state or assumptions about action-result fields.
    slot = (turn.round_no-1)%130
    if not night and role.kind == 'pioneer' and 6 <= slot <= 60 and slot % 6 == 0:
    ...
    for name in sorted(role.backpack, key=lambda n:(not n.startswith('Station'),n)):
        ...
        if unit.kind=='station':
            ...
            threat=sum({'smallRobot':5,'middleRobot':10,'largeRobot':20,'bossRobot':40}.get(r.kind,0)
                       for r in turn.incoming_robots() if distance(r.pos,unit.pos)<=6)
            threshold=max(600*unit.level,(distance(role.pos,unit.pos)+4)*threat)
...
def develop(turn, role, claimed, commands):
    ...
    if role.unit_id==courier.unit_id:
        # Fund the first base upgrade before cheap pressure. The narrow
        # purchase window and carried-stock check allow at most one order/day.
        enemies = [u for u in turn.enemies if u.kind=='station' and u.health>0]
        attack = ('MiddleRobotSummonOrder' if enemies and min(u.health for u in enemies)<900
                  else 'SmallRobotSummonOrder')
        price = turn.shop_prices.get(attack,0)
        repair = 'StationUpgradeVoucher'+str(base.level) if base else ''
        repair_ready = stock[repair]>0 or (base and base.level>=2)
        order_stock = sum(v for n,v in stock.items() if n.endswith('RobotSummonOrder'))
        if (role.kind=='pioneer' and base and base.health>=.65*1500*base.level
                and len(turn.weapons())>=3 and repair_ready and price>0
                and budget>=price and order_stock==0
                and 48 <= (turn.round_no-1)%130 <= 50 and not role.backpack_full):
            if any(distance(role.pos,p)<=1 for p in shops):
