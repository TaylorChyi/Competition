...
from .protocol import Pos, distance, move_command, collect_command
...
def use_carried(turn, role, claimed, commands, night=False):
    ...
    if medicine and role.health < max_hp*.65:
        ...
        return True
    # A single courier and ten final daytime slots guarantee the team daily cap
    # without persistent process state or assumptions about action-result fields.
    slot = (turn.round_no-1)%130
    enemy_exhausted = any(u.kind=='station' and u.level>=3 and 0<u.health<900 for u in turn.enemies)
    if not night and role.kind == 'pioneer' and 60 <= slot < 70 and enemy_exhausted:
        summon = next((n for n in role.backpack if n.endswith('RobotSummonOrder')), None)
        if summon:
            commands[role.unit_id] = {'action':'use','name':summon}
            return True
    ...
    for name in sorted(role.backpack, key=lambda n:(not n.startswith('Station'),n)):
        ...
        if unit.kind=='station':
            if night and distance(role.pos,unit.pos)>1:
                return walk(turn,role,[unit.pos],claimed,commands)
            threat=sum({'smallRobot':5,'middleRobot':10,'largeRobot':20,'bossRobot':40}.get(r.kind,0)
                       for r in turn.incoming_robots() if distance(r.pos,unit.pos)<=3)
            threshold=max(600*unit.level,4*threat)
...
def urgent_refit(turn,role,claimed,commands):
    ...
    base=turn.station()
    if role.kind!='pioneer' or role.backpack_full or not base or base.level>=3 or not (
...
def develop(turn, role, claimed, commands):
    ...
    repair_name = 'StationUpgradeVoucher'+str(station.level) if station and station.level<3 else ''
    repair_reserve = turn.shop_prices.get(repair_name,0) if repair_name and stock[repair_name]==0 else 0
    if (not role.backpack_full and role.health<100 and 'Medicine' in turn.shop_prices
            and budget>=turn.shop_prices['Medicine']+repair_reserve):
    ...
    if role.unit_id==courier.unit_id:
        # Attack only a visibly exhausted enemy base: its level3 upgrade
        # healing is spent. Preserve our next repair and firepower investments.
        vulnerable = any(u.kind=='station' and u.level>=3 and 0<u.health<900 for u in turn.enemies)
        attack = 'MiddleRobotSummonOrder'
        price = turn.shop_prices.get(attack,0)
        repair = 'StationUpgradeVoucher'+str(base.level) if base else ''
        repair_ready = stock[repair]>0 or (base and base.level>=3)
        order_stock = sum(v for n,v in stock.items() if n.endswith('RobotSummonOrder'))
        if (role.kind=='pioneer' and base and base.health>=.8*1500*base.level
                and len(turn.weapons())>=3 and min(t.level for t in turn.weapons())>=2
                and vulnerable and repair_ready and price>0 and budget>=price+150
                and order_stock==0 and 48 <= (turn.round_no-1)%130 <= 50
                and not role.backpack_full):
            if any(distance(role.pos,p)<=1 for p in shops):
                commands[role.unit_id] = {'action':'buy','name':attack,'num':1}
                return True
            return walk(turn,role,shops,claimed,commands)
        for name in goals:
    ...
    def available(p):
        ...
        return p not in claimed and (owner is None or owner.unit_id==role.unit_id)
