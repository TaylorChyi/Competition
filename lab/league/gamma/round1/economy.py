...
def use_carried(turn, role, claimed, commands, night=False):
    ...
    for name in sorted(role.backpack, key=lambda n:(not n.startswith('Station'),n)):
        ...
        if night and unit.kind=='station' and unit.health > 750*unit.level:
            continue
...
def develop(turn, role, claimed, commands):
    ...
    if role.unit_id==courier.unit_id:
        # Only divert surplus after a durable defensive core is funded. Large
        # robots provide 5 HP per gold versus Boss 4; avoid feeding cheap points.
        attack = 'LargeRobotSummonOrder'
        price = turn.shop_prices.get(attack,0)
        towers = turn.weapons()
        if (role.kind=='pioneer' and base and base.health>=.85*1500*base.level and len(towers)>=3
                and (min(t.level for t in towers)>=2 or turn.round_no<=390) and price>0
                and budget>=price+(25 if turn.round_no>390 else 0) and stock[attack]==0
                and (turn.round_no-1)%130<55
