...
def validate_episodes(report, plan, stage, *, complete=False):
    expected = [(case['seed'], case['priority'], seat)
                for case in plan[stage] for seat in plan['seats']]
    actual = [(e['seed'], e['priority'], e['seat']) for e in report['episodes']]
    require(actual == expected[:len(actual)], 'Episodes do not match the planned cases/seats in order')
    if complete:
        require(len(actual) == len(expected), 'Incomplete survival test set')
...
def main():
    ...
    if args.stage == 'validation':
        require(args.development_report, 'Pass the development gate before running reserved validation')
        dev = json.loads(Path(args.development_report).read_text())
        require(dev['stage'] == 'development' and dev['complete'] and dev['passed'],
                'Development survival gate has not passed')
        require(dev['sourceHashes'] == source_hashes, 'Candidate/judge changed after development')
        validate_episodes(dev, plan, 'development', complete=True)
        require(all(passed(e['candidate'], plan['nights']) for e in dev['episodes']),
                'Development report contains a failed episode')
    output = Path(args.output)
    output.parent.mkdir(parents=True, exist_ok=True)
    if output.exists():
        report = json.loads(output.read_text())
        require(report['sourceHashes'] == source_hashes, 'Frozen sources changed; use a new report')
        require(report['stage'] == args.stage, 'Report stage mismatch')
        validate_episodes(report, plan, args.stage)
    else:
    ...
    def save():
        require(hashes(candidate, opponent) == source_hashes, 'Source changed during evaluation')
