...
def develop(turn, role, claimed, commands):
    ...
    def income_rate(p):
        ...
        return amount*turn.sale_prices[turn.zones[p]]/(7+travel+selling)
...
def late_rocket_refit(turn,role,claimed,commands):
    """Convert one level-one gun only with a funded carried upgrade and time."""
    base=turn.station()
    if (not turn.is_day or role.kind!='worker' or role.unit_id in commands
            or not base or base.level!=3 or base.health<=0
            or cash_available(turn,commands)<25
            or len(turn.weapons())!=3 or any(t.kind=='rocket' for t in turn.weapons())):
        return False
    carriers=[u for u in turn.controllable() if 'WeaponUpgradeVoucher1' in u.backpack
              and commands.get(u.unit_id,{}).get('action')!='use']
    daylight=70-(turn.round_no-1)%130
    for tower in sorted(turn.weapons(),key=lambda t:(t.health,t.unit_id)):
        if (tower.kind!='railgun' or tower.level!=1 or tower.pos in claimed
                or distance(role.pos,tower.pos)>1
                or any(c.get('action')=='use' and c.get('targetPos')==[tower.pos.dump()]
                       for c in commands.values())):
            continue
        paths=[route(turn,u,[tower.pos],claimed) for u in carriers]
        if not any(p is not None and len(p)+3<=daylight for p in paths):
            continue
        commands[role.unit_id]={'action':'build','name':'rocket','targetPos':[tower.pos.dump()]}
