...
def use_carried(turn, role, claimed, commands, night=False):
    ...
    for name in sorted(role.backpack, key=lambda n:(not n.startswith('Station'),n)):
        ...
        if not units:
            continue
        unit = min(units, key=lambda u:((u.kind!='railgun' if name=='WeaponUpgradeVoucher1' else u.kind!='rocket'),u.health/(1500*u.level if u.kind=='station' else 500+500*u.level),
...
def develop(turn, role, claimed, commands):
    ...
    if (any(t.kind=='rocket' and t.level==2 for t in turn.weapons())
            and any(t.kind=='railgun' and t.level>=2 for t in turn.weapons())):
