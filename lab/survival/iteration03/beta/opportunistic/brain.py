...
def _worker_day(
    ...
    if economy and turn.shop_prices:
        ...
        if damaged and turn.gold-purchases-planned*WEAPON_BUILD_COST>=WEAPON_BUILD_COST:
            ...
            return
        # Only use already-carried stone at an adjacent front-screen site.
        # No dedicated stone trip or wall repair takes precedence over income.
        if fortify and 'stone' in role.backpack:
            cx,cy=(turn.width-1)/2,(turn.height-1)/2
            front=sorted(_wall_order(turn),key=lambda p:((p.x-cx)**2+(p.y-cy)**2,p.x,p.y))[:fortify]
            for site in front:
                if (site not in claimed and site not in turn.occupied_cells()
                        and role.pos!=site and distance(role.pos,site)<=1):
                    _build_or_walk(turn,role,site,'wall',claimed,commands)
                    return
    if economy and develop(turn,role,claimed,commands):
        return
    if (economy and turn.shop_prices) or not walls_missing:
        return
