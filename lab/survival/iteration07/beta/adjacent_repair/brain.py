...
from .fire_control import targets as defense_targets, joint_railgun_targets
...
def decide(payload: dict[str, Any], *, loadout: tuple[str, ...] | None = None,
           ...
           smart: bool = True, economy: bool = True, fortify: int = 19) -> dict[str, dict[str, Any]]:
...
def _worker_day(
    ...
    if economy and turn.shop_prices:
        ...
        if (fortify and base
            ...
            missing=[p for p in needed if p not in walls]
            if missing:
                needed=missing
            elif 'stone' in role.backpack:
                # Repair an adjacent damaged wall before spending travel actions.
                needed.sort(key=lambda p: not (role.pos != p and distance(role.pos,p)<=1))
            if needed:
...
def _night(turn: Turn, commands: dict[int, dict[str, Any]], policy: dict | None = None,
    ...
    for role, tower in sorted(_tower_pairs(turn, excluded=set(commands)), key=lambda pair: pair[1].range_of_attack()):
        ...
        if distance(role.pos, tower.pos) <= 1:
            ...
            if tower.cooldown > 0:
                continue
            if smart and tower.kind=='railgun':
                pending_railguns.append((role,tower))
                continue
    ...
    plan=joint_railgun_targets(turn,[tower for _,tower in pending_railguns],reserved)
    for role,tower in pending_railguns:
        chosen=plan.get(tower.unit_id,[])
        if chosen:
            command=attack_command(role.unit_id,chosen[0])
            command['targetPos']=[pos.dump() for pos in chosen]
            commands[tower.unit_id]=command
...
def _wall_order(turn: Turn) -> tuple[Pos, ...]:
    ...
    entrances = {Pos(xmax + 2, ymin - 1), Pos(xmin - 2, ymax + 1)}
    rear_x = xmin-2 if station.pos.x < (turn.width-1)/2 else xmax+2
    front_y = ymin-2 if station.pos.y > (turn.height-1)/2 else ymax+2
    rear_y = ymax+2 if station.pos.y > (turn.height-1)/2 else ymin-2
    cells = [pos for pos in order if pos not in entrances and turn.land(pos)]
    # Remove the rear vertical segment, retaining its front corner/flank.
    cells = [pos for pos in cells if not (pos.x==rear_x and pos.y!=front_y)]
    # Extend the omitted rear arc by its next two rear-horizontal cells.
    cells = [pos for pos in cells if not (pos.y==rear_y and abs(pos.x-rear_x) in (1,2))]
    return tuple(cells)
