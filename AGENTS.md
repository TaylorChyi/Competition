...
- 源码和文档纳入 Git；参赛压缩包不提交到源码树。维护者固定待验输出是 `dist/CoreGeek.tar.gz`，该目录被忽略；对外发布时必须改名为 `CoreGeek-vYYYY.MM.DD-<源码短SHA>.tar.gz`，不得上传无版本名附件。改名仅复制字节，不重新打包。
...
- 提交并推送源码后，为该完整提交 SHA 创建带 `bot-` 前缀的新 Release 标签，上传刚才验证过且已加版本号的 `CoreGeek-vYYYY.MM.DD-<源码短SHA>.tar.gz` 并标记 Latest。当前交付的 agent 应完成实际发布，不只停在打包或写说明。
...
- 下载入口固定为 `https://github.com/TaylorChyi/Competition/releases/latest`，最终给用户这个链接或 Latest Release 页面。
...
用户2026-09-18更新优先级：未通过十晚生存验收前，不启动新的三Agent胜率赛。运行 `tools/run_survival_gate.py` 按 `lab/survival/plan.json` 固定开发集，再由同一运行模块与裁判哈希通过独立验证集；失败不得通过删场景、重复已通过场景或换弱对手计为100%。用户已确认机器人每回合移动或攻击二选一，并攻击最近或挡路单位。旧advance/operators属于已被更新事实取代的目标优先假设，保留历史记录但不作为正式机制。只对明确验收集宣称通过，不能泛化成官方100%。
