...
- 同回合分配炮台火力，优先清掉能快速击杀的威胁，减少重复攻击；操作员受到威胁时在炮台附近换位。
- 一名工人维护四格前排围墙，随后参与采矿；开局石矿过远时先赚钱，基地受损时优先升级；建筑损伤跨天保留，残血炮台必要时花金币重建。
- 两名工人分开采矿，按请求中的当前价格卖矿；开拓者购买并运送升级券。受损基地优先升级回血，其余资金逐步升级武器。
- 按实际路线提前回防；碰撞后尝试另一条路，修复采购角色在预留墙线上来回走和工人抢同一矿的问题。

### 最新：完整十晚迭代

本轮不再把“修正选敌”当作生存问题的完成标准。用实际 Bot 指令从 75 金币、零炮台开始，连续运行到第 1300 回合，验证采集、交易、升级、死亡与次日复活。旧版、新版使用同一套模型和波次。
...
这个历史 Demo 是固定装备的单夜火箭防守模拟，未训练神经网络，也未进行玩家间自对弈。新的完整十晚实验见上节；历史范围策略仍可单独试用。
